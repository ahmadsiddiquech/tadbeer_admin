...
<div class="content-wrapper">
    <h3>slider<a href="slider/create"><button type="button" class="btn btn-lg btn-primary pull-right"><i class="fa fa-plus"></i>&nbsp;&nbsp;&nbsp;<b>Add slider</b></button></a></h3>
                        ...
                        <tbody>
                                ...
                                if (isset($news)) {
                                    foreach ($news->result() as
                                            $new) {
                                        $i++;
                                        $set_publish_url = ADMIN_BASE_URL . 'slider/set_publish/' . $new->id;
                                        $set_unpublish_url = ADMIN_BASE_URL . 'slider/set_unpublish/' . $new->id ;
                                        $edit_url = ADMIN_BASE_URL . 'slider/create/' . $new->id ;
                                        $delete_url = ADMIN_BASE_URL . 'slider/delete/' . $new->id;
                                        ?>
                                    <tr id="Row_<?=$new->id?>" class="odd gradeX " >
                                        ...
                                        <td><?php echo substr($new->description_1, 0, 50)?></td>
                                        ...
                                        echo anchor($edit_url, '<i class="fa fa-edit"></i>', array('class' => 'action_edit btn blue c-btn','title' => 'Edit slider'));

                                        echo anchor('"javascript:;"', '<i class="fa fa-times"></i>', array('class' => 'delete_record btn red c-btn', 'rel' => $new->id, 'title' => 'Delete slider'));
                                        ?>
                                        </td>
                                    </tr>
                                    <?php } ?>    
...
$(document).ready(function(){
    ...
    var id = $(this).attr('rel');
        $.ajax({
            type: 'POST',
            url: "<?php ADMIN_BASE_URL?>slider/detail",
            data: {'id': id},
            async: false,
          ...
          $(document).off('click', '.delete_record').on('click', '.delete_record', function(e){
                ...
                e.preventDefault();
              swal({
                title : "Are you sure to delete the selected slider?",
                ...
                function () {
                       $.ajax({
                            type: 'POST',
                            url: "<?php echo ADMIN_BASE_URL?>slider/delete",
                            data: {'id': id},
                            async: false,
                            success: function() {
                            location.reload();
        ...
        $(document).off("click",".action_publish").on("click",".action_publish", function(event) {
            ...
            var status = $(this).attr('status');
             $.ajax({
                type: 'POST',
                url: "<?= ADMIN_BASE_URL ?>slider/change_status",
                ...
                success: function(result) {
                    ...
                    $("#listing").load('<?php ADMIN_BASE_URL?>slider/manage');
