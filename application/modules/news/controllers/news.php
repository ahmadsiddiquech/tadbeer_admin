...
//Modules::run('site_security/has_permission');
    ...
    function manage() {
        $data['news'] = $this->_get('news.id desc');
        $data['view_file'] = 'news';
    ...
    function submit() {
            ...
            if ($update_id != 0) {
                ...
                $actual_img_old = FCPATH . 'uploads/news/actual_images/' . $itemInfo->image;
                $medium_img_old = FCPATH . 'uploads/news/medium_images/' . $itemInfo->image;
                $large_img_old = FCPATH . 'uploads/news/large_images/' . $itemInfo->image;
    ...
    function delete() {
        ...
        $file = $itemInfo->image;
        unlink('./uploads/news/medium_images/' . $file);
        unlink('./uploads/news/large_images/' . $file);
        unlink('./uploads/news/actual_images/' . $file);
    ...
    function set_publish() {
        ...
        redirect(ADMIN_BASE_URL . 'news/manage/' . '');
    }
    ...
    function upload_image($nId) {
        ...
        if(isset($upload_image_file) && !empty($upload_image_file)){
            ...
            $file_name = 'news_' . $nId . $upload_image_file;
        }
        else{
            $file_name = '';
        }
        $config['upload_path'] = './uploads/news/actual_images';
        ...
        $config['width'] = 500;
        $config['height'] = 400;
        $config['new_image'] = './uploads/news/large_images';
        ...
        $config['width'] = 300;
        $config['height'] = 200;
        $config['new_image'] = './uploads/news/medium_images';
        $this->image_lib->initialize($config);
        $this->image_lib->resize();
        $this->image_lib->clear();
        ...
        $config['width'] = 100;
        $config['height'] = 100;
        $config['new_image'] = './uploads/news/small_images';
        $this->image_lib->initialize($config);
        $this->image_lib->resize();
        $this->image_lib->clear();
    ...
    function _get($order_by) {
        $this->load->model('mdl_news');
        return $this->mdl_news->_get($order_by);
    }
