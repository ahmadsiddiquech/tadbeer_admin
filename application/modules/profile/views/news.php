...
<div class="content-wrapper">
    <h3>Services<a href="services/create"><button type="button" class="btn btn-lg btn-primary pull-right"><i class="fa fa-plus"></i>&nbsp;&nbsp;&nbsp;<b>Add Services</b></button></a></h3>
                        ...
                        <tbody>
                                ...
                                if (isset($news)) {
                                    foreach ($news->result() as
                                            $new) {
                                        $i++;
                                        $set_publish_url = ADMIN_BASE_URL . 'services/set_publish/' . $new->id;
                                        $set_unpublish_url = ADMIN_BASE_URL . 'services/set_unpublish/' . $new->id ;
                                        $edit_url = ADMIN_BASE_URL . 'services/create/' . $new->id ;
                                        $delete_url = ADMIN_BASE_URL . 'services/delete/' . $new->id;
                                        ?>
...
$(document).ready(function(){
    ...
    var id = $(this).attr('rel');
        $.ajax({
            type: 'POST',
            url: "<?php ADMIN_BASE_URL?>services/detail",
            data: {'id': id},
            async: false,
          ...
          $(document).off('click', '.delete_record').on('click', '.delete_record', function(e){
              ...
              swal({
                ...
                function () {
                       $.ajax({
                            type: 'POST',
                            url: "<?php echo ADMIN_BASE_URL?>services/delete",
                            data: {'id': id},
                            async: false,
                            success: function() {
                            location.reload();
            ...
            var status = $(this).attr('status');
             $.ajax({
                ...
                success: function(result) {
                    ...
                    $("#listing").load('<?php ADMIN_BASE_URL?>services/manage');
