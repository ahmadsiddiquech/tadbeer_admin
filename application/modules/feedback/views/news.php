                        ...
                        <tbody>
                                ...
                                if (isset($news)) {
                                    foreach ($news->result() as
                                            $new) {
                                        $i++;
                                        $set_publish_url = ADMIN_BASE_URL . 'feedback/set_publish/' . $new->id;
                                        $set_unpublish_url = ADMIN_BASE_URL . 'feedback/set_unpublish/' . $new->id ;
                                        $edit_url = ADMIN_BASE_URL . 'feedback/create/' . $new->id ;
                                        $delete_url = ADMIN_BASE_URL . 'feedback/delete/' . $new->id;
                                        ?>
    ...
    var id = $(this).attr('rel');
          ...
          $(document).off('click', '.delete_record').on('click', '.delete_record', function(e){
              ...
              swal({
                ...
                function () {
                       $.ajax({
                            type: 'POST',
                            url: "<?php echo ADMIN_BASE_URL?>feedback/delete",
                            data: {'id': id},
                            async: false,
                            success: function() {
                            location.reload();
        ...
        $(document).off("click",".action_publish").on("click",".action_publish", function(event) {
            ...
            var status = $(this).attr('status');
             $.ajax({
                type: 'POST',
                url: "<?= ADMIN_BASE_URL ?>feedback/change_status",
                ...
                success: function(result) {
                    ...
                    $("#listing").load('<?php ADMIN_BASE_URL?>feedback/manage');
