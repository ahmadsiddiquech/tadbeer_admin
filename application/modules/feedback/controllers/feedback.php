...
//Modules::run('site_security/has_permission');
    ...
    function set_publish() {
        ...
        redirect(ADMIN_BASE_URL . 'feedback/manage/' . '');
    }
    ...
    function _getItemById($id) {
        ...
        return $this->mdl_feedback->_getItemById($id);
    }

    function _set_publish($arr_col) {
        $this->load->model('mdl_feedback');
        $this->mdl_feedback->_set_publish($arr_col);
    }

    function _set_unpublish($arr_col) {
        $this->load->model('mdl_feedback');
        $this->mdl_feedback->_set_unpublish($arr_col);
    }
    ...
    function _update($arr_col,$data) {
        $this->load->model('mdl_feedback');
        $this->mdl_feedback->_update($arr_col, $data);
    }

    function _update_id($id, $data) {
        $this->load->model('mdl_feedback');
        $this->mdl_feedback->_update_id($id, $data);
    }

    function _delete($arr_col) {       
        $this->load->model('mdl_feedback');
        $this->mdl_feedback->_delete($arr_col);
    }
}