                        ...
                        <tbody>
                                ...
                                if (isset($news)) {
                                    foreach ($news->result() as
                                            $new) {
                                        $i++;
                                        $set_publish_url = ADMIN_BASE_URL . 'contact_us/set_publish/' . $new->id;
                                        $set_unpublish_url = ADMIN_BASE_URL . 'contact_us/set_unpublish/' . $new->id ;
                                        $edit_url = ADMIN_BASE_URL . 'contact_us/create/' . $new->id ;
                                        $delete_url = ADMIN_BASE_URL . 'contact_us/delete/' . $new->id;
                                        ?>
                                    <tr id="Row_<?=$new->id?>" class="odd gradeX " >
                                        ...
                                        <td><?php echo substr($new->message, 0, 30)?></td>
...
$(document).ready(function(){
    ...
    var id = $(this).attr('rel');
        $.ajax({
            type: 'POST',
            url: "<?php ADMIN_BASE_URL?>contact_us/detail",
            data: {'id': id},
            async: false,
          ...
          $(document).off('click', '.delete_record').on('click', '.delete_record', function(e){
              ...
              swal({
                ...
                function () {
                       $.ajax({
                            type: 'POST',
                            url: "<?php echo ADMIN_BASE_URL?>contact_us/delete",
                            data: {'id': id},
                            async: false,
                            success: function() {
                            location.reload();
        ...
        $(document).off("click",".action_publish").on("click",".action_publish", function(event) {
            ...
            var status = $(this).attr('status');
             $.ajax({
                type: 'POST',
                url: "<?= ADMIN_BASE_URL ?>contact_us/change_status",
                ...
                success: function(result) {
                    ...
                    $("#listing").load('<?php ADMIN_BASE_URL?>contact_us/manage');
