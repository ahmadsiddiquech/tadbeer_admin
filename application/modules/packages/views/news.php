...
<div class="content-wrapper">
    <h3>Packages<a href="packages/create"><button type="button" class="btn btn-lg btn-primary pull-right"><i class="fa fa-plus"></i>&nbsp;&nbsp;&nbsp;<b>Add Packages</b></button></a></h3>
                        ...
                        <tbody>
                                ...
                                if (isset($news)) {
                                    foreach ($news->result() as
                                            $new) {
                                        $i++;
                                        $set_publish_url = ADMIN_BASE_URL . 'packages/set_publish/' . $new->id;
                                        $set_unpublish_url = ADMIN_BASE_URL . 'packages/set_unpublish/' . $new->id ;
                                        $edit_url = ADMIN_BASE_URL . 'packages/create/' . $new->id ;
                                        $delete_url = ADMIN_BASE_URL . 'packages/delete/' . $new->id;
                                        ?>
...
$(document).ready(function(){
    ...
    var id = $(this).attr('rel');
        $.ajax({
            type: 'POST',
            url: "<?php ADMIN_BASE_URL?>packages/detail",
            data: {'id': id},
            async: false,
          ...
          $(document).off('click', '.delete_record').on('click', '.delete_record', function(e){
              ...
              swal({
                ...
                function () {
                       $.ajax({
                            type: 'POST',
                            url: "<?php echo ADMIN_BASE_URL?>packages/delete",
                            data: {'id': id},
                            async: false,
                            success: function() {
                            location.reload();
        ...
        $(document).off("click",".action_publish").on("click",".action_publish", function(event) {
            ...
            var status = $(this).attr('status');
             $.ajax({
                type: 'POST',
                url: "<?= ADMIN_BASE_URL ?>packages/change_status",
                ...
                success: function(result) {
                    ...
                    $("#listing").load('<?php ADMIN_BASE_URL?>packages/manage');
